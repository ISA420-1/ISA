...
function PIXEL.DrawSimpleText(text, font, x, y, col, xAlign, yAlign)
    if text == nil then return 0, 0 end -- Added check for nil text
    setFont(font)
...
function PIXEL.DrawText(text, font, x, y, col, xAlign, yAlign)
    if text == nil then return end -- Added check for nil text
    local curX = x
    local curY = y
...
function PIXEL.DrawDualText(title, subtitle, x, y, h)
    if title[1] == nil or subtitle[1] == nil then return end -- Added check for nil text
    x = x or 0
    y = y or 0
