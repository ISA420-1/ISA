...
hook.Add("entity_killed", "PIXEL.HUD.NLRTimer", function(data)
    ...
    hook.Add("HUDPaint", "PIXEL.HUD.NLRTimer", function()
        local titleY = PIXEL.Scale(localPly:Team() == TEAM_SPECTATOR and 160 or 80)
        local _, titleH = PIXEL.DrawSimpleText("NEW LIFE RULE", "HUD.NLRTitle", centerX, titleY, titleCol, textAlignCenter)
        PIXEL.DrawSimpleText("You must not return to your death location for another " .. math.Round(finishTime - CurTime()) .. " seconds.", "HUD.NLRDescription", centerX, titleY + titleH, descriptionCol, textAlignCenter)
    end)
