...
-- Add a helper function to check if a player has a gang
local overheadsEnabled = CreateClientConVar("pixel_hud_overheads_enabled", "1", true, false, "Should overheads be shown?", 0, 1)

-- Define a helper function to check if a player has a gang
local function PlayerHasGang(ply)
    -- Implement the logic to check if the player has a gang.
    -- Return true if the player has a gang, otherwise return false.
    -- You can modify this logic based on how gangs are stored and checked in your system.
    -- Example: return ply:HasGang() -- Implement this method if it exists
    return false -- Placeholder, replace with actual logic
end

PIXEL.HUD.SetupOverheadDrawing = function()
    ...
    local stats = {
        ...
        { -- Gang
            ...
            color = colors.Gang,
            getter = function(ply)
                if PlayerHasGang(ply) then
                    local gangInfo = BRS_PLYGANGINFO and BRS_PLYGANGINFO[ply:SteamID()] or false
                    if gangInfo and gangInfo.Name then
                        return gangInfo.Name
                    else
                        return "N/A"
                    end
                else
