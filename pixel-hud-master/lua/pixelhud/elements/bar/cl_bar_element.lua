...
function PANEL:SizeElement(overrideText)
    local textToMeasure = overrideText or self.Text or ""  -- Ensure a default value if overrideText or self.Text is nil
    if textToMeasure == "0" then 
        self:SetWide(0) 
        return 
    end

    self:SetWide(self:GetTall() + PIXEL.GetTextSize(textToMeasure, "HUD.BarElement") + PIXEL.Scale(8))
end
